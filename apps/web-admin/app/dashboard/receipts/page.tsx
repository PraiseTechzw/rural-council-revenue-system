...
export default function ReceiptsPage() {
  ...
  return (
    <section className="dashboard-page reveal">
      <header>
        <h1 className="dashboard-title">Receipts Vault</h1>
        <p className="dashboard-subtitle">Instant lookup and recent issue trail for verification and auditing workflows.</p>
      </header>
      ...
      <div className="premium-panel p-4">
        ...
        {lookupNumber ? (
          <div className="mt-4 rounded-xl border border-[#ddcfb3] bg-[#f8f0e0] p-3">
            ...
            {lookupQuery.data ? (
              <div className="space-y-1 text-sm text-slate-800">
                ...
                <Link href={`/dashboard/receipts/${lookupQuery.data.id}`} className="inline-block pt-1 text-sm font-semibold text-[#1e3a27]">
                  Open detail
                </Link>
              </div>
            ) : null}
            ...
            <tbody>
              {recentPaymentsQuery.isLoading ? (
                ...
                recentPaymentsQuery.data.rows
                  ...
                  .map((payment) => (
                    <tr key={payment.id} className="border-t border-[#e9decb]">
                      ...
                      <td className="px-3 py-2 text-slate-700">
                        <Link href={`/dashboard/receipts/${payment.receipt.id}`} className="font-semibold text-[#24442f] hover:text-[#182f20]">
                          View
                        </Link>
                      </td>
                    </tr>
